      ...
      <form onSubmit={handleSubmit} className="TaskEditor">
        ...
        {todo.end != null ? (
          <DateTimePicker
            ...
            title={'New due date: '}
            value={value}
            defaultDate={dueDate.slice(0, 10)}
            defaultTime={dueDate.slice(11, 19)}
          />
        ) : (
          <DateTimePicker
            onDateTimeChange={handleDateTimeChange}
        ...
        <button type="submit" className="TaskEditButton">
          Update Task
        </button>
      </form>
      <Button
        onClick={() => deleteTaskFunc(todo.id)}
        icon={<img src={trash_icon} alt="icon" style={{ width: '20px', height: '20px' }} />}
      />
    </>
  )
}
